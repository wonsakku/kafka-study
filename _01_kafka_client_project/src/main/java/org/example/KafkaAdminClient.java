...
//        configs.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, BOOTSTRAP_SERVERS);
        configs.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, List.of(BOOTSTRAP_SERVERS));
        final AdminClient admin = AdminClient.create(configs);
        logger.info("-- Get broker information");

        for (Node node : admin.describeCluster().nodes().get()) {
            logger.info("node : {}", node);
            final ConfigResource cr = new ConfigResource(ConfigResource.Type.BROKER, node.idString());
